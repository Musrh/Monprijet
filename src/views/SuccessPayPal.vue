<template>
  <div class="p-6 text-center">
    <h1 class="text-2xl font-bold text-green-600 mb-4">Paiement PayPal réussi ✅</h1>

    <p v-if="loading">Confirmation du paiement en cours...</p>

    <p v-if="success" class="text-green-600 font-semibold">
      Votre commande a été enregistrée avec succès.
    </p>

    <p v-if="error" class="text-red-600 font-semibold">
      Une erreur est survenue lors de la validation.
    </p>
  </div>
</template>

<script>
import axios from "axios";
import { mapState } from "vuex";

export default {
  name: "SuccessPayPal",
  data() {
    return { loading: true, success: false, error: false };
  },
  computed: {
    ...mapState(["cart", "user"]),
  },
  async mounted() {
    const token = this.$route.query.token; // token PayPal

    if (!token) {
      this.error = true;
      this.loading = false;
      return;
    }

    try {
      await axios.post(
        "https://paypalbackend-production.up.railway.app/capture-paypal-order",
        {
          orderId: token,
          email: this.user.email,
          items: this.cart,
          adresseLivraison: this.$store.state.adresseLivraison || "", // ou computed depuis Panier.vue
        }
      );

      this.success = true;
    } catch (err) {
      console.error("Erreur capture PayPal:", err);
      this.error = true;
    } finally {
      this.loading = false;
    }
  },
};
</script>
