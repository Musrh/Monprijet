<template>
  <div class="home">
    <!-- Slider -->
    <SliderProducts v-if="sliderProduits.length" :produits="sliderProduits" />

    <!-- Vedette + promos côte à côte -->
    <div class="flex gap-8 my-8" v-if="produitVedette || produitsPromo.length">
    ...
    <!-- Popup promo -->
    <div v-if="showPopup && popupProduit" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      selectVedetteEtPromo();
    });

    return { sliderProduits, produitVedette, produitsPromo, ventes, showPopup, popupProduit, ajouterAuPanier, closePopup };
  }
};
</script>
...
.badge { background-color: red; color: white; padding: 2px 6px; border-radius: 4px; font-size: 0.75rem; }
</style>
