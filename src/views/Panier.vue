    ...
    <div v-else>
      ...
      <div class="mt-4">
        ...
        <select v-model="paymentMethod" class="border p-2 rounded w-full">
          ...
          <option value="paypal" :disabled="!user">
            🅿️ PayPal
          </option>

        </select>

        <p v-if="!user" class="text-sm text-red-500 mt-1">
          Connectez-vous pour utiliser PayPal
        </p>
...
export default {
  ...
  watch: {
    ...
    paymentMethod(newMethod) {
      ...
      if (newMethod === "paypal" && !this.user) {

        alert("Veuillez vous connecter pour utiliser PayPal");
