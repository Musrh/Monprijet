<template>
  <div class="p-4 max-w-3xl mx-auto">
    ...
    <div v-else>
      ...
      <!-- Bouton Payer Stripe -->
      <button v-if="paymentMethod === 'stripe'" @click="payerStripe" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded mt-4 w-full">
        💳 Payer avec Stripe
      </button>
...
export default {
  ...
  computed: {
    ...mapState(["cart", "user"]),
    total() { return this.cart.reduce((sum, item) => sum + item.prix * item.quantity, 0); }
  },
  watch: {
  ...
  methods: {
    ...
    async payerStripe() {
      ...
      const itemsPourCommande = this.cart.map(p => ({ id: p.id, nom: p.nom, prix: p.prix, quantity: p.quantity, image: p.images?.[0] || p.image || "/placeholder.png" }));
    ...
    async renderPaypalButton() {
      ...
      const itemsPourCommande = this.cart.map(p => ({ id: p.id, nom: p.nom, prix: p.prix, quantity: p.quantity }));
